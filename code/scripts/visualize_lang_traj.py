...
class CustomModel:
    def __init__(self, cfg):
        state_dict = torch.load(f'/iliad/u/manasis/language-diffuser/code/logs/checkpoint/state_debug_dataset_100epochs.pt',
...
def rollout(env, model, task_oracle, args, subtask, lang_embeddings, val_annotations, plans):
    ...
    if True: #subtask == 'push_red_block_right':
        print("\n\n\nSUBTASK: ", subtask, "\n\n\n")
        x, y, z = np.array(x), np.array(y), np.array(z)
        ax.scatter(x,y,z)
        plt.show()
        plt.savefig('/iliad/u/manasis/language-diffuser/code/debug_images/img_test.png')
        import pdb;pdb.set_trace()
