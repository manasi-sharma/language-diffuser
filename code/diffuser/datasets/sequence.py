...
class SequenceDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, #env='hopper-medium-replay',
        ...
        for i, batch in enumerate(calvin_dataloader):
            ...
            #import pdb;pdb.set_trace()
            perceptual_emb = model.perceptual_encoder.proprio_encoder(np.concatenate( (batch_obj["robot_obs"], batch_obj["state_info"]['scene_obs']), axis=2 ) ).squeeze().detach().numpy() #torch.Size([32, 32, 3, 200, 200]) --> torch.Size([32, 32, 72])
